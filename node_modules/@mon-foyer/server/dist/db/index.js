...
async function initDb() {
  ...
  `);
    ...
    const migV1 = await db.get("SELECT value FROM app_settings WHERE key='migration_v1'");
    if (!migV1) {
    ...
    const propCount = await db.get('SELECT COUNT(*) as n FROM rental_properties');
    if (Number(propCount?.n ?? 0) === 0) {
        await db.run('INSERT INTO rental_properties (name, credit_label, credit_amount, rent_label, rent_amount) VALUES (?, ?, ?, ?, ?)', 'Appartement 09/10', 'Crédit LCL 09/10', 927.71, 'Loyer locataire', 1500.00);
        await db.run('INSERT INTO rental_properties (name, credit_label, credit_amount, rent_label, rent_amount) VALUES (?, ?, ?, ?, ?)', 'Appartement 20/21', 'Crédit LCL 20/21', 337.77, 'CIP Gestion', 465.91);
    }
    // Seed recurring transactions
    const recCount = await db.get('SELECT COUNT(*) as n FROM recurring_transactions');
    if (Number(recCount?.n ?? 0) === 0) {
        ...
        for (const s of seeds) {
            await db.run('INSERT INTO recurring_transactions (label, amount, type, category, default_day) VALUES (?, ?, ?, ?, ?)', s.label, s.amount, s.type, s.category, s.day);
        }
    }
}
