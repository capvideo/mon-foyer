...
async function seed() {
  ...
  `);
    ...
    const hash = await bcryptjs_1.default.hash('Foyer2026!', 10);
    ...
    for (const t of txs) {
        await db.run('INSERT INTO transactions (account_id,label,amount,type,category,date,validated,member_id,notes,is_rental) VALUES (?,?,?,?,?,?,?,?,?,?)', t);
    }
    // Events
    const events = [
    ...
    for (const e of events) {
        await db.run('INSERT INTO events (title,date,time,end_date,end_time,description,member_ids,color,location,reminder_minutes) VALUES (?,?,?,?,?,?,?,?,?,?)', e);
    }
    // Shopping items
    const items = [
    ...
    for (const i of items) {
        await db.run('INSERT INTO shopping_items (name,category,quantity,unit,checked,added_by,created_at) VALUES (?,?,?,?,?,?,?)', [...i, now]);
    }
    // Todos
    const todos = [
    ...
    for (const t of todos) {
        await db.run('INSERT INTO todos (title,description,priority,assigned_to,status,due_date,event_id,created_at) VALUES (?,?,?,?,?,?,?,?)', [...t, now]);
    }
    // Channels
    const channels = [
    ...
    for (const m of msgs) {
        await db.run('INSERT INTO messages (channel_id,member_id,content,type,timestamp,metadata) VALUES (?,?,?,?,?,?)', m);
    }
