...
async function getOpeningBalance(db, accountId, month) {
    const stored = await db.get('SELECT opening_balance FROM monthly_balances WHERE account_id = ? AND month = ?', accountId, month);
    if (stored)
        return stored.opening_balance;
  ...
  `, accountId, `${prevMonth}-01`, `${month}-01`);
...
router.post('/', async (req, res) => {
    ...
    const r = await db.run('INSERT INTO accounts (name, bank, initial_balance, color) VALUES (?, ?, ?, ?)', name, bank, initialBalance ?? 0, color ?? '#378ADD');
    res.status(201).json(await db.get('SELECT * FROM accounts WHERE id = ?', r.lastID));
});
router.put('/:id', async (req, res) => {
    const db = await (0, index_1.getDb)();
    const { name, bank, initialBalance, color } = req.body;
    await db.run('UPDATE accounts SET name=?, bank=?, initial_balance=?, color=? WHERE id=?', name, bank, initialBalance, color, req.params.id);
...
router.post('/:id/monthly-balance', async (req, res) => {
    ...
    await db.run(`INSERT INTO monthly_balances (account_id, month, opening_balance) VALUES (?, ?, ?)
     ON CONFLICT (account_id, month) DO UPDATE SET opening_balance = EXCLUDED.opening_balance`, req.params.id, month, opening_balance);
